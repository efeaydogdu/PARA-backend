        ...
        lines.forEach(line => {
            ...
            if (/^(Name|Student Number|Birth Date|Admit Semester|Admit Type|Level|Faculty|Program\(s\))/.test(line)) {
                ...
                if (key !== 'Admit Semester') {
                    studentInfo[key.toLowerCase().replace(/\(s\)/, 's').replace(/\s/g, '_')] = value;
                }
            } else if (/^(Fall|Spring|Summer)\s\d{4}-\d{4}/.test(line) && !line.includes('Admit Semester')) {
...
function parseCurrentCourseLine(line) {
    ...
    const ectsCredits = creditsMatch[2].substring(0, 2) + creditsMatch[2].substring(2); // Insert dot
...
function parseCourseLine(line) {
    ...
    const creditsString = line.substring(ugIndexFromEnd + grade.length + 2).trim(); // +2 to skip "UG"
    ...
    let qualityEctsString = creditsParts[1] + (creditsParts[2] ? '.' + creditsParts[2] : ''); // Handle missing parts gracefully
    ...
    const ectsCredits = qualityEctsString.substring(dotIndex + 3).replace(/^\.+/, ''); // Remove any leading dots
...
// Specify the path to your PDF. Make sure to use double backslashes for Windows paths or single forward slashes for other OS
